...
// Purpose: 
//-----------------------------------------------------------------------------
static void SetMaster_f(  )
{
	master->SetMaster_f();
}

//-----------------------------------------------------------------------------
// Purpose: Add master servers
//-----------------------------------------------------------------------------
CON_COMMAND(addmaster, "Add master servers"){
	int count;
	count = args.ArgC();
	netadr_t adr;

	if (count < 1){
		Msg("Usage: addmaster <ip-address>\n");
		return;
	} else {
		// Convert to netadr_t
		if ( NET_StringToAdr ( args.ArgS(), &adr ) )
		{
			// Add to master list
